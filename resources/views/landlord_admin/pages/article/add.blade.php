                ...
                <div class="card-body">
                    <form action="{{ route('landlord_admin.article.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row gy-4">
                            ...
                            <div class="col-md-6">
                                <label for="type" class="form-label">Loại Tin <span style="color: red">*</span></label>
                                <select name="type" class="form-select">
                                    <option value="regular" {{ old('type') == 'regular' ? 'selected' : '' }}>Tin thường</option>
                                    <option value="vip" {{ old('type') == 'vip' ? 'selected' : '' }}>Tin VIP</option>
                                    <option value="urgent" {{ old('type') == 'urgent' ? 'selected' : '' }}>Tin gấp</option>
                                    <option value="free" {{ old('type') == 'free' ? 'selected' : '' }}>Tin miễn phí</option>
                                </select>
                                @error('type')
                                    <p style="color: red;">{{ $message }}</p>
                                @enderror
